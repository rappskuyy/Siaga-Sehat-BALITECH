...
Tolong lakukan AI Health Assessment dan kembalikan JSON.`;
  ...
  if (!text) {
    console.warn("KoboiLLM returned no readable completion", {
      keys: payload && typeof payload === "object" ? Object.keys(payload) : [],
    });
    throw new Error("KoboiLLM tidak mengembalikan respon valid.");
  }
...
function getCompletionText(payload: unknown): string | null {
  ...
  for (const candidate of candidates) {
    const text = textFromCompletionValue(candidate);
    if (text) return text;
  }
...
function textFromCompletionValue(value: unknown): string | null {
  ...
  const text = value
    .map((part) => {
      ...
      const item = part as { text?: unknown; content?: unknown; output_text?: unknown };
